...
function setState(){
    ...
    if (in_array($user_response['user_id'], $ticked)) {
        $output = array('1', $response);
        include 'chat.php';
        $surpress = sortByType('create', $response["fb_id"]);
    }
    /*
    foreach(count($ticked) as $tickedUser) {
        if ($tickedUser == $user_id){
            $output = array('1', $response);
            include 'chat.php';
            sortByType('create', $response["user_id"]);
        }
    }
    */
    $output = $output;
